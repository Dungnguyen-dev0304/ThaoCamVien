...
- **Global scope**: `D:/ThaoCamVien/ThaoCamVien/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/ThaoCamVien/ThaoCamVien/.claude/notes/`)
...
2. Glob `D:/ThaoCamVien/ThaoCamVien/.claude/notes/*.md` (if directory exists) — global notes
...
Global (D:/ThaoCamVien/ThaoCamVien/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/ThaoCamVien/ThaoCamVien/.claude/notes/` — works in any directory
